        ...
        <div className="flex h-screen bg-slate-50 font-sans text-slate-800 overflow-hidden selection:bg-yellow-300 selection:text-slate-900">
            ...
            <aside className="hidden lg:flex w-72 bg-white border-r border-slate-200 flex-col z-20 shrink-0">
                <div className="p-8 pb-4 flex flex-col items-center border-b border-slate-100">
                    ...
                    <p className="text-[9px] text-slate-400 uppercase tracking-widest text-center mt-1 font-bold">CED Registrar System <br /> Admin Portal</p>
                </div>
                    ...
                    {menuItems.map((item) => {
                        ...
                        return (
                            <Link
                                key={item.name}
                                href={item.link}
                                className={`w-full text-left px-5 py-3 rounded-2xl font-bold transition-all flex items-center gap-3 text-sm ${isActive
                                        ? 'bg-amber-50/60 text-amber-500'
                                        : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900'
                                    }`}
                            >
                                <svg className={`w-5 h-5 shrink-0 ${isActive ? 'text-amber-500' : 'text-slate-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className={`fixed inset-y-0 left-0 z-50 w-80 bg-white shadow-2xl transform transition-transform duration-300 ease-in-out lg:hidden flex flex-col ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                <button onClick={() => setIsSidebarOpen(false)} className="absolute top-4 right-4 text-slate-400 hover:text-slate-600 p-2">
